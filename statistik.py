# statistik.py — återställt GB i "vanlig" statistik + prognos kvar
...
def compute_stats(rows_df: pd.DataFrame, cfg: dict) -> dict:
    ...
    Numerik visas med svenska tusentals-mellanrum och kommatecken som decimal.
    Innehåller:
      • Vanlig statistik (inkl. GB-sektion)
      • Prognos 365d (inkl. GB 365d och nöjdhet/vecka)
    """
    out = {}

    # ===== Hjälpare =====
    def _fmt2(x) -> str:
        # Svensk formatering: 12 345,67
        try:
            val = float(x)
    ...
    TID_D_SEK     = _col("Tid D")  # sparas som "Tid D"
    ...
    # ===== GB-sektion (tillbaka i "vanlig" statistik) =====
    out["— GB —"] = ""
    ...
    _sum_snitt_tillfallen(LBL_BEK,     BE, MAX_BEK if MAX_BEK > 0 else 1.0)
    _sum_snitt_tillfallen(LBL_ESK,     ES, 1.0)
    _sum_snitt_tillfallen("Bonus deltagit", BD, 1.0)
    _sum_snitt_tillfallen("Personal deltagit", PD, max(1.0, float(cfg.get("PROD_STAFF", 1))))
    ...
    til_pappan  = _antal_tillfallen(sum_pappan,  MAX_PAPPAN)
    til_grannar = _antal_tillfallen(sum_grannar, MAX_GRANNAR)
    til_nv      = _antal_tillfallen(sum_nv,      MAX_NV)
    til_nf      = _antal_tillfallen(sum_nf,      MAX_NF)
    sover_div_max_nf = _div(sum_sover, MAX_NF)

    out["— Nöjdhet —"] = ""
    out[f"Nöjdhet – {LBL_PAPPAN}"]  = _fmt2(alskar_snitt_kanner + til_pappan)
    out[f"Nöjdhet – {LBL_GRANNAR}"] = _fmt2(alskar_snitt_kanner + til_grannar)
    out[f"Nöjdhet – {LBL_NV}"]      = _fmt2(alskar_snitt_kanner + til_nv)
    out[f"Nöjdhet – {LBL_NF}"]      = _fmt2(alskar_snitt_kanner + til_nf + sover_div_max_nf)
    ...
    # Känner tjänar (nu): (Intäkt Känner + Vinst – summa) / (MAX_PAPPAN+MAX_GRANNAR+MAX_NV+MAX_NF+MAX_BEKANTA)
    denom_kanner_tot = MAX_PAPPAN + MAX_GRANNAR + MAX_NV + MAX_NF + MAX_BEK
    out["Känner tjänar (nu)"] = _fmt2(_div(float(IK.sum()) + float(V.sum()), denom_kanner_tot))

    # ===== Prognos 365d =====
    out["— Prognos 365d —"] = ""
    n = max(1, len(rows_df)) if rows_df is not None else 1
    f = 365.0 / n  # skalfaktor

    # Totalt män, Svarta
    out["Totalt antal män – 365d"] = _fmt2(total_man_sum * f)
    out["Summa Svarta (inkl. regler) – 365d"] = _fmt2(sum_black * f)
    out["Andel Svarta (%) – 365d"] = _fmt2(100.0 * _div(sum_black, total_man_sum))

    # Super bonus ack prognos
    sb_pct = float(cfg.get("SUPER_BONUS_PCT", 0.1)) / 100.0
    super_now = float(PREN.sum()) * sb_pct
    out["Super-bonus ack – prognos (365d)"] = _fmt2(super_now * f)

    # Älskar/Sover (summa och “snitt”-mått från ovan)
    out["Älskar – summa (365d)"] = _fmt2(float(ALSKAR.sum()) * f)
    out["Snitt älskar (Älskar snitt känner) – 365d"] = _fmt2(alskar_snitt_kanner)

    out["Sover med – summa (365d)"] = _fmt2(float(SOVER.sum()) * f)
    out["Snitt sover med (Sover med / MAX (Nils familj)) – 365d"] = _fmt2(sover_div_max_nf)

    # Källor (summa + snitt per scen)
    def _prog_block(label, ser, maxv):
        out[f"{label} – summa (365d)"] = _fmt2(float(ser.sum()) * f)
        cnt  = _count_rows(ser > 0)
        avg  = _div(float(ser.sum()), cnt)
        out[f"{label} – snitt per scen (365d)"] = _fmt2(avg)

    _prog_block(LBL_PAPPAN,  P,  MAX_PAPPAN)
    _prog_block(LBL_GRANNAR, G,  MAX_GRANNAR)
    _prog_block(LBL_NV,      NV, MAX_NV)
    _prog_block(LBL_NF,      NF, MAX_NF)
    _prog_block("Bonus deltagit", BD, 1.0)
    _prog_block("Personal deltagit", PD, max(1.0, float(cfg.get("PROD_STAFF", 1))))
    _prog_block(LBL_BEK, BE, MAX_BEK if MAX_BEK>0 else 1.0)
    _prog_block(LBL_ESK, ES, 1.0)

    # Ekonomi (365d)
    out["Prenumeranter – summa (365d)"] = _fmt2(float(PREN.sum()) * f)
    out["Intäkter – summa (365d)"]      = _fmt2(float(INT.sum()) * f)
    out["Kostnad män – summa (365d)"]   = _fmt2(float(KM.sum()) * f)
    out["Intäkt Känner – summa (365d)"] = _fmt2(float(IK.sum()) * f)
    out["Intäkt företag – summa (365d)"]= _fmt2(float(IF.sum()) * f)
    out["Lön Malin – summa (365d)"]     = _fmt2(float(LM.sum()) * f)
    out["Vinst – summa (365d)"]         = _fmt2(float(V.sum()) * f)

    out["Känner tjänar (365d)"] = _fmt2(_div((float(IK.sum()) + float(V.sum())) * f, denom_kanner_tot))

    # GB 365d
    out["— GB (365d) —"] = ""
    out["Antal GB (365d)"]         = _fmt2(cnt_gb * f)
    out["Privat GB (365d)"]        = _fmt2(cnt_privat * f)
    out["Antal GB vita (365d)"]    = _fmt2(cnt_gb_vita * f)
    out["Antal GB svarta (365d)"]  = _fmt2(cnt_gb_svarta * f)
    out["Antal GB blandat (365d)"] = _fmt2(cnt_gb_blandat * f)

    # Prognos Nöjdhet per vecka (365d -> /52)
    out["— Prognos Nöjdhet (per vecka) —"] = ""
    per_week = 365.0/52.0/n  # ~7.02/n, dvs vi tar dagsnivå * 7/vecka
    out[f"{LBL_PAPPAN} – nöjdhet/vecka (prognos)"]  = _fmt2((alskar_snitt_kanner + til_pappan) * per_week * n)
    out[f"{LBL_GRANNAR} – nöjdhet/vecka (prognos)"] = _fmt2((alskar_snitt_kanner + til_grannar) * per_week * n)
    out[f"{LBL_NV} – nöjdhet/vecka (prognos)"]      = _fmt2((alskar_snitt_kanner + til_nv) * per_week * n)
    out[f"{LBL_NF} – nöjdhet/vecka (prognos)"]      = _fmt2((alskar_snitt_kanner + til_nf + sover_div_max_nf) * per_week * n)
    out["Nils – per vecka (prognos)"]               = _fmt2((sum_nils / max(1, n)) * 7.0)
