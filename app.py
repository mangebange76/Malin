...
def läs_inställningar(sh):
    ...
    if "Namn" not in df.columns or "Värde" not in df.columns:
        st.error("Bladet 'Inställningar' saknar kolumner.")
        return {}
    ...
    for i in range(0, len(värden), max_rader_per_update):
        ...
        for rad in chunk:
            if len(rad) != len(COLUMNS):
                st.error(f"⚠️ Rad med fel kolumnantal ({len(rad)}): {rad}")
                return
...
def scenformulär(df, inst, sh):
    with st.form("lägg_till_scen"):
        ...
        st.markdown(f"**Minuter per kille:** {round(tid_per_kille_min, 2)} min")
        st.markdown(f"**Total tid för scen:** {round(total_tid_h, 2)} h")
...
def inställningspanel(sh, inst):
    ...
    with st.sidebar.form("inst"):
        f = {}
        f["Startdatum"] = st.text_input("Startdatum", inst.get("Startdatum", "2014-03-26"))
        f["Kvinnans namn"] = st.text_input("Kvinnans namn", inst.get("Kvinnans namn", "Malin"))
        f["Födelsedatum"] = st.text_input("Födelsedatum", inst.get("Födelsedatum", "1984-03-26"))
...
def main():
    st.set_page_config("Malin-produktionsapp", layout="wide")
