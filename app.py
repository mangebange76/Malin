...
# Autentisering och kalkylarks-URL
...
# Rubriker / kolumnstruktur
RUBRIKER = [
    "Datum", "Typ", "DP", "DPP", "DAP",
    "TPA", "TPP", "TAP", "Enkel vaginal", "Enkel anal",
    "Kompisar", "Pappans vänner", "Nils vänner", "Nils familj", "Övriga män",
    "Älskar med", "Sover med", "Nils sex", "DT tid per man (sek)",
    "DT total tid (sek)", "Total tid (sek)", "Total tid (h)",
    "Prenumeranter", "Intäkt ($)", "Kvinnans lön ($)", "Mäns lön ($)",
    "Kompisars lön ($)", "Minuter per kille", "Scenens längd (h)"
]

INST_KOL = ["Inställning", "Värde", "Senast ändrad"]

# Initiera blad om de inte finns
def init_sheet(name, cols):
    try:
        ...
        if worksheet.row_values(1) != cols:
            worksheet.clear()
            worksheet.update("A1", [cols])
...
def init_inställningar():
    try:
        ...
        worksheet.update("A2:C8", standard)
...
def spara_inställning(nyckel, värde):
    ...
    if nyckel in df["Inställning"].values:
        rad = df[df["Inställning"] == nyckel].index[0] + 2
        worksheet.update_cell(rad, 2, str(värde))
        worksheet.update_cell(rad, 3, idag)
    else:
...
def load_data():
    try:
        ...
        data = worksheet.get_all_records()
        if not data:
            return pd.DataFrame(columns=RUBRIKER)
