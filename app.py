...
def _retry_call(fn, *args, **kwargs):
    """Exponential backoff för 429/RESOURCE_EXHAUSTED — inga skrivningar här."""
    delay = 0.5
    for _ in range(6):
        try:
...
def get_client():
    ...
    # Vi kommer inte att skriva något förrän användaren trycker "Spara raden".
...
# Öppna Data-blad – inga skrivningar, och vi försöker inte skapa nya blad
try:
    sheet = _retry_call(spreadsheet.worksheet, WORKSHEET_TITLE)
except Exception as e:
    st.error(f"Kunde inte öppna bladet '{WORKSHEET_TITLE}'. Skapa det i dokumentet och ladda om.\n\n{e}")
    st.stop()

# =========================== Header-läsning (ingen migration/skrivning) =========================
DEFAULT_COLUMNS = [
    ...
    "Pappans vänner","Grannar","Nils vänner","Nils familj","Bekanta","Eskilstuna killar","Bonus killar",
...
try:
    ...
    st.error(f"Kunde inte läsa header från '{WORKSHEET_TITLE}': {e}")
    st.stop()

if not header:
    st.error(f"Bladet '{WORKSHEET_TITLE}' saknar header. Lägg in kolumnerna:\n{', '.join(DEFAULT_COLUMNS)}")
    st.stop()

KOLUMNER = header  # vi skriver INTE in default; bara använder det som referens

# ============================ Etiketter från 'Inställningar' ============================
@st.cache_data(show_spinner=False, ttl=30)
def _load_labels():
    """
    Läser etiketter från fliken 'Inställningar' (Key/Value/Label).
    Returnerar dict: {Key -> Label}; om Label tomt används Key.
    Skriver aldrig till arket.
    """
    labels = {}
    try:
        ws = _retry_call(spreadsheet.worksheet, SETTINGS_SHEET)
    except Exception:
        # Fliken saknas -> använd fallback (labels = tom => Key används)
        return labels
    ...
    for r in rows:
        ...
        if key:
            labels[key] = lbl if lbl else key
    return labels

LABELS = _load_labels()

def L(key: str) -> str:
    """Hämta etikett för ett fältnamn (key). Etiketten påverkar inte beräkningarna."""
...
if view == "Statistik":
    ...
    antal_scener = 0
    privat_gb_cnt = 0
    totalt_man = 0  # summerar Män + Eskilstuna killar (dina tidigare regler)
    ...
    for r in rows:
        ...
        men_like = man + esk  # för scener och totals (överenskommet tidigare)
    ...
    with c1: st.metric(L("Antal scener"), antal_scener)
    with c2: st.metric(L("Privat GB"), privat_gb_cnt)
    with c3: st.metric(L("Totalt antal män"), totalt_man)
    with c4: st.metric(L("Snitt scener"), snitt_scener)
    with c5: st.metric(L("Snitt Privat GB"), snitt_privat_gb)
    st.metric(L("Andel svarta av män (%)"), andel_svarta_pct)

    # --- Maxvärden i session (labels påverkar bara UI) ---
    max_p  = int(st.session_state.get("MAX_PAPPAN", 10))
    max_g  = int(st.session_state.get("MAX_GRANNAR", 10))
    max_nv = int(st.session_state.get("MAX_NILS_VANNER", 10))
    max_nf = int(st.session_state.get("MAX_NILS_FAMILJ", 10))
    ...
    st.markdown("---")
    st.subheader("📐 " + L("Snitt (rel. max) + Totalt antal tillfällen"))
    cA, cB = st.columns(2)
    with cA:
        st.markdown(f"**{L('Pappans vänner')}**")
        st.metric(L("Snitt (rel. max)"), pv_avg_rel)
        st.metric(L("Totalt antal tillfällen"), pv_tot_tillf)

        st.markdown(f"**{L('Grannar')}**")
        st.metric(L("Snitt (rel. max)"), gr_avg_rel)
        st.metric(L("Totalt antal tillfällen"), gr_tot_tillf)
    with cB:
        st.markdown(f"**{L('Nils vänner')}**")
        st.metric(L("Snitt (rel. max)"), nv_avg_rel)
        st.metric(L("Totalt antal tillfällen"), nv_tot_tillf)

        st.markdown(f"**{L('Nils familj')}**")
        st.metric(L("Snitt (rel. max)"), nf_avg_rel)
        st.metric(L("Totalt antal tillfällen"), nf_tot_tillf)
    ...
    st.markdown("---")
    st.subheader("👥 " + L("Prenumeranter"))
    pc1, pc2 = st.columns(2)
    with pc1: st.metric(L("Prenumeranter (totalt)"), int(total_pren))
    with pc2: st.metric(L("Aktiva prenumeranter (30 dagar)"), int(aktiva_pren))
    ...
    st.markdown("---")
    st.subheader("💰 " + L("Ekonomi (totalt)"))
    ec1, ec2 = st.columns(2)
    with ec1: st.metric(L("Intäkt känner (totalt)"), f"{round(total_intakt_kanner, 2)} USD")
    with ec2: st.metric(L("Intäkt företag (totalt)"), f"{round(total_intakt_foretag, 2)} USD")
    ec3, ec4 = st.columns(2)
    with ec3: st.metric(L("Vinst (totalt)"), f"{round(total_vinst, 2)} USD")
    with ec4: st.metric(L("Lön Malin (totalt)"), f"{round(total_lon_malin, 2)} USD")

    # Snitt intäkt känner (tidigare överenskommelse)
    ...
    st.metric(L("Snitt intäkt känner"), f"{snitt_intakt_kanner:.2f} USD")
    ...
    st.metric(L("Snitt lön"), f"{snitt_lon:.2f} USD")

    # --- DP/DPP/DAP/TAP ---
    st.markdown("---")
    st.subheader("🔩 " + L("DP / DPP / DAP / TAP — summa & snitt"))
    ...
    with s1: st.metric(L("Summa DP (>0)"), dp_sum)
    with s2: st.metric(L("Summa DPP (>0)"), dpp_sum)
    with s3: st.metric(L("Summa DAP (>0)"), dap_sum)
    with s4: st.metric(L("Summa TAP (>0)"), tap_sum)

    a1, a2, a3, a4 = st.columns(4)
    with a1: st.metric(L("Snitt DP / scen"), dp_avg)
    with a2: st.metric(L("Snitt DPP / scen"), dpp_avg)
    with a3: st.metric(L("Snitt DAP / scen"), dap_avg)
    with a4: st.metric(L("Snitt TAP / scen"), tap_avg)

    # --- Älskar / Sover med + Nils-summa + per dag ---
    st.markdown("---")
    st.subheader("💗 " + L("Älskar / 😴 Sover med — summa & snitt (plus Nils-summa)"))
    ...
    with c_als1: st.metric(L("Summa Älskar"), alskar_sum)
    with c_als2: st.metric(L("Snitt Älskar"), snitt_alskar2)
    with c_sov1: st.metric(L("Summa Sover med"), sover_sum)
    with c_sov2: st.metric(L("Snitt Sover med"), snitt_sover2)
    st.metric(L("Nils (summa)"), nils_sum)
    ...
    d1, d2 = st.columns(2)
    with d1: st.metric(L("Älskar / dag"), f"{alskar_per_dag:.2f}")
    with d2: st.metric(L("Sover med / dag"), f"{sover_per_dag:.2f}")

    # --- Snitt tid kille / scen ---
    st.markdown("---")
    st.subheader("⏱️ " + L("Tid per kille / scen"))
    ...
    st.metric(L("Snitt tid kille / scen"), tpk_avg_label)
    ...
    st.metric(L("Snitt tid (h) per scen – exkl. älskar & sover med"), f"{snitt_tid_h_utan_extra:.2f} h")

    st.stop()

# ================================ Sidopanel (Produktion) ====================================
st.sidebar.header(L("Inställningar"))
...
startdatum = st.sidebar.date_input(L("Historiskt startdatum"), value=_clamp(CFG["startdatum"], MIN_START, date(2100,1,1)))
starttid   = st.sidebar.time_input(L("Starttid"), value=CFG["starttid"])
födelsedatum = st.sidebar.date_input(
    L("Malins födelsedatum"),
    value=_clamp(CFG["födelsedatum"], MIN_FOD, date.today()),
    min_value=MIN_FOD, max_value=date.today()
)

st.sidebar.subheader(L("Maxvärden (Auto-Max med varning)"))
max_p  = st.sidebar.number_input(f"{L('Pappans vänner')} (max)", min_value=0, step=1, value=int(CFG["MAX_PAPPAN"]))
max_g  = st.sidebar.number_input(f"{L('Grannar')} (max)",        min_value=0, step=1, value=int(CFG["MAX_GRANNAR"]))
max_nv = st.sidebar.number_input(f"{L('Nils vänner')} (max)",    min_value=0, step=1, value=int(CFG["MAX_NILS_VANNER"]))
max_nf = st.sidebar.number_input(f"{L('Nils familj')} (max)",    min_value=0, step=1, value=int(CFG["MAX_NILS_FAMILJ"]))
max_bk = st.sidebar.number_input(f"{L('Bekanta')} (max)",        min_value=0, step=1, value=int(CFG["MAX_BEKANTA"]))

st.sidebar.subheader(L("Pris per prenumerant (gäller NÄSTA rad)"))
avgift_input = st.sidebar.number_input(L("Avgift (USD, per ny rad)"), min_value=0.0, step=1.0, value=float(CFG["avgift_usd"]))

# "Spara inställningar" sparar *endast i session* här — inga skrivningar till ark.
if st.sidebar.button("💾 " + L("Spara inställningar")):
    CFG.update({
    ...
    st.success(L("Inställningar sparade (lokalt i session) ✅"))
...
# ===== 30 dagar (rullande) i sidopanelen (read-only) =====
st.sidebar.subheader("📆 " + L("30 dagar (rullande)"))
try:
    ...
    for r in all_rows:
        ...
        active_subs += subs
        active_rev  += subs * fee
    st.sidebar.metric(L("Aktiva prenumeranter"), int(active_subs))
    st.sidebar.metric(L("Intäkter (30 dagar)"), f"${active_rev:,.2f}")
except Exception as e:
...
# ============================ Inmatning (live-fält) ============================
st.subheader("➕ " + L("Lägg till ny händelse"))

män    = st.number_input(L("Män"),    min_value=0, step=1, value=0)
svarta = st.number_input(L("Svarta"), min_value=0, step=1, value=0)
fitta  = st.number_input(L("Fitta"),  min_value=0, step=1, value=0)
rumpa  = st.number_input(L("Rumpa"),  min_value=0, step=1, value=0)
...
tid_s  = st.number_input(L("Tid S (sek)"), min_value=0, step=1, value=60)
tid_d  = st.number_input(L("Tid D (sek)"), min_value=0, step=1, value=60)
vila   = st.number_input(L("Vila (sek)"),  min_value=0, step=1, value=7)

dt_tid  = st.number_input(L("DT tid (sek/kille)"),  min_value=0, step=1, value=60)
dt_vila = st.number_input(L("DT vila (sek/kille)"), min_value=0, step=1, value=3)

älskar    = st.number_input(L("Älskar"),                min_value=0, step=1, value=0)
sover_med = st.number_input(L("Sover med (0 eller 1)"), min_value=0, max_value=1, step=1, value=0)

lbl_p  = f"{L('Pappans vänner')} (max {st.session_state.MAX_PAPPAN})"
lbl_g  = f"{L('Grannar')} (max {st.session_state.MAX_GRANNAR})"
lbl_nv = f"{L('Nils vänner')} (max {st.session_state.MAX_NILS_VANNER})"
lbl_nf = f"{L('Nils familj')} (max {st.session_state.MAX_NILS_FAMILJ})"
...
eskilstuna_killar = st.number_input(L("Eskilstuna killar"), min_value=0, step=1, value=0, key="input_eskilstuna")
nils = st.number_input(L("Nils"), min_value=0, step=1, value=0)
...
grund_preview = {
    ...
    "Nils vänner": nils_vänner, "Nils familj": nils_familj, "Bekanta": bekanta,
    "Eskilstuna killar": eskilstuna_killar,
    "Nils": nils,
...
st.markdown("---")
st.subheader("🔎 " + L("Förhandsvisning (innan spar)"))

col1, col2 = st.columns(2)
with col1:
    st.metric(L("Datum / veckodag"), f"{rad_datum} / {veckodag}")
    st.metric(L("Summa tid"), preview.get("Summa tid", "-"))
    st.metric(L("Summa tid (sek)"), int(preview.get("Summa tid (sek)", 0)))
    st.metric(L("Hångel (m:s/kille)"), preview.get("Hångel (m:s/kille)", "-"))
with col2:
    st.metric(L("Totalt män (raden)"), int(preview.get("Totalt Män", 0)))
    st.metric(L("Tid per kille"), preview.get("Tid per kille", "-"))
    st.metric(L("Tid per kille (sek)"), int(preview.get("Tid per kille (sek)", 0)))
    st.metric(L("Suger per kille (sek)"), int(preview.get("Suger per kille (sek)", 0)))

st.caption(f"{L('Klockan blir')}: {preview.get('Klockan','-')} ({L('start')} {starttid})")

# ===== Prenumeranter & Ekonomi (live) — enbart visning =====
st.markdown("#### 📈 " + L("Prenumeranter & Ekonomi (live)"))
ec1, ec2, ec3, ec4 = st.columns(4)
with ec1:
    st.metric(L("Prenumeranter (rad)"), int(preview.get("Prenumeranter", 0)))
    st.metric(L("Avgift (rad)"), _usd(preview.get("Avgift", CFG['avgift_usd'])))
with ec2:
    st.metric(L("Intäkter (rad)"), _usd(preview.get("Intäkter", 0)))
    st.metric(L("Lön Malin"), _usd(preview.get("Lön Malin", 0)))
with ec3:
    st.metric(L("Kostnad män"), _usd(preview.get("Intäkt män", 0)))
    st.metric(L("Intäkt Känner"), _usd(preview.get("Intäkt Känner", 0)))
with ec4:
    st.metric(L("Intäkt Företaget"), _usd(preview.get("Intäkt Företaget", 0)))
    st.metric(L("Vinst (rad)"), _usd(preview.get("Vinst", 0)))
...
def _apply_auto_max_and_save(pending):
    # Uppdatera lokala max i session (ej i arket) och spara raden
...
save_clicked = st.button("💾 " + L("Spara raden"))
if save_clicked:
    over_max = {}
    if pappans_vänner > st.session_state.MAX_PAPPAN:
...
# Auto-Max dialog (ingen skrivning förrän användaren bekräftar)
if "PENDING_SAVE" in st.session_state:
    pending = st.session_state["PENDING_SAVE"]
    st.warning(L("Du har angett värden som överstiger max. Vill du uppdatera maxvärden (lokalt) och spara raden?"))
    for f, info in pending["over_max"].items():
        st.write(f"- **{L(f)}**: max {info['current_max']} → **{info['new_value']}**")

    c1, c2 = st.columns(2)
    with c1:
        if st.button("✅ " + L("Ja, uppdatera max och spara")):
            try:
                ...
                st.rerun()
    with c2:
        if st.button("✋ " + L("Nej, avbryt")):
            st.session_state.pop("PENDING_SAVE", None)
            st.info(L("Sparning avbröts. Justera värden eller max i sidopanelen."))

# ============================== Snabbåtgärder (read-only tills spara) ===============================
st.markdown("---")
st.subheader("🛠️ " + L("Snabbåtgärder"))
...
# 'Vila på jobbet' — skapar beräknad rad, men skriver inte förrän du sparar manuellt (via _save_row om du vill koppla)
if st.button("➕ " + L("Skapa 'Vila på jobbet'-rad (förhands)")):
    try:
        ...
        grund_vila = {
            ...
            "Nils vänner": nv, "Nils familj": nf, "Bekanta": bk,
            "Eskilstuna killar": esk,
            "Nils": 0,
            "Avgift": float(CFG.get("avgift_usd", 30.0)),
        }
        prev = _calc_preview(grund_vila)
        st.info(L("Förhandsrad skapad — inget sparat ännu. Använd 'Spara raden' för att skriva till databasen."))
        st.json(prev)
    except Exception as e:
        st.error(f"Misslyckades att skapa förhandsrad: {e}")

# 'Vila i hemmet' — motsvarande förhandsvisning
if st.button("🏠 " + L("Skapa 'Vila i hemmet' (7 dagar, förhands)")):
    try:
        ...
        for offset in range(7):
            ...
            previews.append(_calc_preview(grund_home))

        st.info(L("Förhandsrader (7 st) skapade — inget sparat ännu."))
        for p in previews:
            st.json(p)
    except Exception as e:
        st.error(f"Misslyckades att skapa förhandsrader: {e}")

# ================================ Visa & radera ================================
st.subheader("📊 " + L("Aktuella data"))
try:
    rows = _retry_call(sheet.get_all_records)
    if rows:
        st.dataframe(rows, use_container_width=True)
    else:
        st.info(L("Inga datarader ännu."))
except Exception as e:
    st.warning(f"Kunde inte läsa data: {e}")

st.subheader("🗑 " + L("Ta bort rad"))
try:
    total_rows = st.session_state.ROW_COUNT
    if total_rows > 0:
        idx = st.number_input(L("Radnummer att ta bort (1 = första dataraden)"), min_value=1, max_value=total_rows, step=1, value=1)
        if st.button(L("Ta bort vald rad")):
            # Skrivning sker endast här vid radering
            _retry_call(sheet.delete_rows, int(idx) + 1)  # +1 för header
            st.session_state.ROW_COUNT -= 1
            st.success(f"{L('Rad')} {idx} {L('borttagen')}.")
            st.rerun()
    else:
        st.caption(L("Ingen datarad att ta bort."))
except Exception as e:
