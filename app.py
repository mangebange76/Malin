...
SETTINGS_SHEET = "Inställningar"  # exact english headers: Key / Value / Label
...
def _get_settings_ws():
    try:
        ...
        # Create with header Key/Value/Label
...
def _save_config(d, labels):
    ws = _get_settings_ws()
    # read existing
    rows = _retry_call(ws.get_all_records)
    existing = { (r.get("Key") or "").strip(): r for r in rows }
    # build up map of all keys
    all_keys = set(list(d.keys()) + list(labels.keys()))
    out = [["Key","Value","Label"]]
    for k in sorted(all_keys):
        v = d.get(k, "")
        l = labels.get(k, existing.get(k, {}).get("Label",""))
        out.append([k, v, l])
    _retry_call(ws.clear)
    _retry_call(ws.update, f"A1:C{len(out)}", out)

def _load_cfg_and_labels():
    d, labels = _config_as_dict()
    # defaults if missing
    defaults = {
    ...
    for k,v in defaults.items():
        if k not in d or str(d[k]).strip() == "":
            d[k] = v
            updated = True
    if updated:
        _save_config(d, labels)
    return d, labels
...
# ============ Ladda konfig + etiketter (permanent i Inställningar) =============
CFG_STR, LABELS = _load_cfg_and_labels()

def _label_for(key, fallback):
    # For the MAX_* keys, we prefer their label; otherwise allow direct label entries.
    if key in LABELS and LABELS[key].strip():
        return LABELS[key].strip()
    return fallback
...
if view == "Statistik":
    ...
    # --- Basmetrik: scener, privat GB, totalt män (män+svarta+esk+source+bekanta+bonus), snitt scener, snitt privat GB ---
    antal_scener = 0
    privat_gb_cnt = 0
    totalt_man = 0
    ...
    total_svarta_sum = 0
    total_men_like_sum = 0  # män+svarta+esk+source+bekanta (+bonus) for %
    ...
    for r in rows:
        ...
        pv = _safe_int(r.get("Pappans vänner", 0), 0)
        gr = _safe_int(r.get("Grannar", 0), 0)
        nv = _safe_int(r.get("Nils vänner", 0), 0)
        nf = _safe_int(r.get("Nils familj", 0), 0)
        bk = _safe_int(r.get("Bekanta", 0), 0)
        bonus_d = _safe_int(r.get("Bonus deltagit", 0), 0)

        men_like = man + svarta + esk + pv + gr + nv + nf + bk + bonus_d
    ...
    max_p  = int(CFG_STR.get("MAX_PAPPAN", "0"))
    max_g  = int(CFG_STR.get("MAX_GRANNAR", "0"))
    max_nv = int(CFG_STR.get("MAX_NILS_VANNER", "0"))
    max_nf = int(CFG_STR.get("MAX_NILS_FAMILJ", "0"))
    max_bk = int(CFG_STR.get("MAX_BEKANTA", "0"))
    ...
    bk_sum = sum(_safe_int(r.get("Bekanta", 0), 0) for r in rows)
    ...
    bk_avg_rel = round(bk_sum / max_bk, 2) if max_bk > 0 else 0.0
    ...
    # --- Ekonomi (totalsummor i USD) ---  (oförändrat enligt överenskommelse)
    ...
    tpk_total_sec = sum(_safe_int(r.get("Tid per kille (sek)", 0)) for r in rows if (_safe_int(r.get("Totalt Män", 0)) > 0))
    ...
    total_sec_scen = sum(
        ...
        for r in rows if _safe_int(r.get("Totalt Män", 0)) > 0
    )
    alskar_sec_scen = sum(
...
def _init_cfg_defaults_from_sheet():
    st.session_state.setdefault("CFG", {})

    # Parse config values
    st.session_state["CFG"]["startdatum"] = _parse_iso_date(CFG_STR.get("startdatum", date.today().isoformat())) or date.today()
    try:
        st.session_state["CFG"]["starttid"] = datetime.strptime(CFG_STR.get("starttid","07:00:00"), "%H:%M:%S").time()
    except Exception:
        st.session_state["CFG"]["starttid"] = time(7,0)
    st.session_state["CFG"]["födelsedatum"] = _parse_iso_date(CFG_STR.get("födelsedatum", date(1990,1,1).isoformat())) or date(1990,1,1)

    st.session_state["CFG"]["MAX_PAPPAN"]      = int(CFG_STR.get("MAX_PAPPAN","10"))
    st.session_state["CFG"]["MAX_GRANNAR"]     = int(CFG_STR.get("MAX_GRANNAR","10"))
    st.session_state["CFG"]["MAX_NILS_VANNER"] = int(CFG_STR.get("MAX_NILS_VANNER","10"))
    st.session_state["CFG"]["MAX_NILS_FAMILJ"] = int(CFG_STR.get("MAX_NILS_FAMILJ","10"))
    st.session_state["CFG"]["MAX_BEKANTA"]     = int(CFG_STR.get("MAX_BEKANTA","10"))
    try:
        st.session_state["CFG"]["avgift_usd"] = float(CFG_STR.get("avgift_usd","30"))
    except Exception:
        st.session_state["CFG"]["avgift_usd"] = 30.0

    st.session_state["CFG"]["BONUS_TOTAL"] = int(CFG_STR.get("BONUS_TOTAL","0"))
    st.session_state["CFG"]["BONUS_USED"]  = int(CFG_STR.get("BONUS_USED","0"))

_init_cfg_defaults_from_sheet()
CFG = st.session_state["CFG"]

startdatum = st.sidebar.date_input(
    _label_for("startdatum","Historiskt startdatum"),
    value=_clamp(CFG["startdatum"], MIN_START, date(2100,1,1))
)
starttid   = st.sidebar.time_input(
    _label_for("starttid","Starttid"),
    value=CFG["starttid"]
)
födelsedatum = st.sidebar.date_input(
    _label_for("födelsedatum","Malins födelsedatum"),
...
max_p  = st.sidebar.number_input(_label_for("MAX_PAPPAN","Max Pappans vänner"), min_value=0, step=1, value=int(CFG["MAX_PAPPAN"]))
max_g  = st.sidebar.number_input(_label_for("MAX_GRANNAR","Max Grannar"),        min_value=0, step=1, value=int(CFG["MAX_GRANNAR"]))
max_nv = st.sidebar.number_input(_label_for("MAX_NILS_VANNER","Max Nils vänner"),    min_value=0, step=1, value=int(CFG["MAX_NILS_VANNER"]))
max_nf = st.sidebar.number_input(_label_for("MAX_NILS_FAMILJ","Max Nils familj"),    min_value=0, step=1, value=int(CFG["MAX_NILS_FAMILJ"]))
max_bk = st.sidebar.number_input(_label_for("MAX_BEKANTA","Max Bekanta"),        min_value=0, step=1, value=int(CFG["MAX_BEKANTA"]))

st.sidebar.subheader("Pris per prenumerant (gäller NÄSTA rad)")
avgift_input = st.sidebar.number_input(_label_for("avgift_usd","Avgift (USD, per ny rad)"), min_value=0.0, step=1.0, value=float(CFG["avgift_usd"]))
...
if st.sidebar.button("💾 Spara inställningar"):
    ...
    # Persist to Inställningar
    new_d = dict(CFG_STR)
    new_d.update({
        "startdatum": startdatum.isoformat(),
        "starttid": starttid.strftime("%H:%M:%S"),
        "födelsedatum": födelsedatum.isoformat(),
        "MAX_PAPPAN": str(int(max_p)),
        "MAX_GRANNAR": str(int(max_g)),
        "MAX_NILS_VANNER": str(int(max_nv)),
        "MAX_NILS_FAMILJ": str(int(max_nf)),
        "MAX_BEKANTA": str(int(max_bk)),
        "avgift_usd": str(float(avgift_input)),
        "BONUS_TOTAL": str(int(CFG.get("BONUS_TOTAL",0))),
        "BONUS_USED": str(int(CFG.get("BONUS_USED",0))),
    })
    _save_config(new_d, LABELS)
    st.success("Inställningar sparade ✅")

# Se till att max finns i session (för etiketter och labels i inputs)
...
def datum_och_veckodag_för_scen(scen_nummer: int):
    d = st.session_state["CFG"]["startdatum"] + timedelta(days=scen_nummer - 1)
...
män    = st.number_input(_label_for("label_män","Män"),    min_value=0, step=1, value=0)
svarta = st.number_input(_label_for("label_svarta","Svarta"), min_value=0, step=1, value=0)
...
lbl_p  = f"{_label_for('MAX_PAPPAN','Pappans vänner')} (max {st.session_state.MAX_PAPPAN})"
lbl_g  = f"{_label_for('MAX_GRANNAR','Grannar')} (max {st.session_state.MAX_GRANNAR})"
lbl_nv = f"{_label_for('MAX_NILS_VANNER','Nils vänner')} (max {st.session_state.MAX_NILS_VANNER})"
lbl_nf = f"{_label_for('MAX_NILS_FAMILJ','Nils familj')} (max {st.session_state.MAX_NILS_FAMILJ})"
lbl_bk = f"{_label_for('MAX_BEKANTA','Bekanta')} (max {st.session_state.MAX_BEKANTA})"
...
eskilstuna_killar = st.number_input(_label_for("label_eskilstuna","Eskilstuna killar"), min_value=0, step=1, value=0, key="input_eskilstuna")
...
grund_preview = {
    ...
    "Bonus deltagit": 0,  # preview: none consumed yet
}

def _calc_preview(grund):
    try:
        if callable(calc_row_values):
            return calc_row_values(grund, rad_datum, st.session_state["CFG"]["födelsedatum"], st.session_state["CFG"]["starttid"])
        else:
...
with ec1:
    ...
    st.metric("Avgift (rad)", _usd(preview.get("Avgift", st.session_state['CFG']['avgift_usd'])))
with ec2:
...
def _save_row(grund, rad_datum, veckodag, apply_bonus_award=True):
    try:
        base = dict(grund)
        base.setdefault("Avgift", float(st.session_state["CFG"]["avgift_usd"]))
        ber = calc_row_values(base, rad_datum, st.session_state["CFG"]["födelsedatum"], st.session_state["CFG"]["starttid"])
        ber["Datum"] = rad_datum.isoformat()
        # Bonus (award only on normal save, not on 'vila' rows which handle participation separately)
        bonus_awarded = 0
        if apply_bonus_award:
            ...
            if pren > 0:
                ...
                for _ in range(pren):
                    if random.random() < 0.05:
                        bonus_awarded += 1
        ber["Bonus killar"] = bonus_awarded
        ber["Bonus deltagit"] = int(base.get("Bonus deltagit", 0) or 0)
        # persist
        row = [ber.get(col, "") for col in KOLUMNER]
        _retry_call(sheet.append_row, row)
        st.session_state.ROW_COUNT += 1
        # Update BONUS_TOTAL/USED in settings
        new_total = int(st.session_state["CFG"]["BONUS_TOTAL"]) + int(bonus_awarded)
        new_used  = int(st.session_state["CFG"]["BONUS_USED"]) + int(ber["Bonus deltagit"])
        st.session_state["CFG"]["BONUS_TOTAL"] = new_total
        st.session_state["CFG"]["BONUS_USED"]  = new_used
        # Persist to settings
        nd = dict(CFG_STR)
        nd.update({
            "BONUS_TOTAL": str(new_total),
            "BONUS_USED": str(new_used),
        })
        _save_config(nd, LABELS)

        ålder = rad_datum.year - st.session_state["CFG"]["födelsedatum"].year - ((rad_datum.month,rad_datum.day)<(st.session_state["CFG"]["födelsedatum"].month,st.session_state["CFG"]["födelsedatum"].day))
...
def _apply_auto_max_and_save(pending):
    # update max in session + settings if needed
    cfg_sheet = dict(CFG_STR)
    for _, info in pending["over_max"].items():
        new_val = int(info["new_value"])
        st.session_state[info["max_key"]] = new_val
        st.session_state["CFG"][info["max_key"].replace("MAX_","MAX_")] = new_val
        cfg_sheet[info["max_key"]] = str(new_val)
    _save_config(cfg_sheet, LABELS)
...
def _bonus_remaining():
    return max(0, int(st.session_state["CFG"]["BONUS_TOTAL"]) - int(st.session_state["CFG"]["BONUS_USED"]))
...
if st.button("➕ Skapa 'Vila på jobbet'-rad"):
    try:
        ...
        # Allocate 40% of remaining bonus pool to this row
        rem = _bonus_remaining()
        alloc = math.floor(rem * 0.40) if rem > 0 else 0
        ...
        grund_vila = {
            ...
            "Nils": 0,
            "Avgift": float(st.session_state["CFG"].get("avgift_usd", 30.0)),
            "Bonus deltagit": alloc,
        }
        _save_row(grund_vila, rad_datum2, veckodag2, apply_bonus_award=False)  # no awarding on "vila"
    except Exception as e:
...
if st.button("🏠 Skapa 'Vila i hemmet' (7 dagar)"):
    try:
        ...
        # Distribute ALL remaining bonus evenly across day1-5
        rem = _bonus_remaining()
        per_day = rem // 5 if rem > 0 else 0
        remainder = rem % 5 if rem > 0 else 0
        ...
        for offset in range(7):
            ...
            if offset <= 4:
                ...
                esk = _rand_eskilstuna_20_40()
                # bonus share
                bonus_share = per_day + (1 if offset < remainder else 0)
            else:
                pv = gr = nv = nf = bk = 0
                esk = _rand_eskilstuna_20_40()
                bonus_share = 0
            ...
            grund_home = {
                ...
                "Nils": nils_val,
                "Avgift": float(st.session_state["CFG"].get("avgift_usd", 30.0)),
                "Bonus deltagit": bonus_share,
            }
            _save_row(grund_home, rad_d, veckod, apply_bonus_award=False)

        st.success("✅ Skapade 7 'Vila i hemmet'-rader (Nils 50/45/5 + bonusfördelning).")
    except Exception as e:
