...
from konstanter import COLUMNS, bestäm_datum
...
def beräkna_tid_per_kille(f):
    ...
    dt_tid_total = f.get("DT tid per man (sek)", 0) * antal_killar
    tid_älskar = f.get("Älskar med", 0) * 30 * 60  # 30 minuter per älskare
    total_tid = dt_tid_total + tid_älskar
    tid_per_kille = total_tid / antal_killar

    return tid_per_kille / 60, total_tid / 3600  # minuter och timmar

def process_lägg_till_rader(df, inst, f):
    tid_per_kille_min, total_tid_h = beräkna_tid_per_kille(f)
    dt_tid_total = f.get("DT tid per man (sek)", 0) * (
        f.get("Övriga män", 0)
        + f.get("Enkel vaginal", 0)
        + f.get("Enkel anal", 0)
        + f.get("DP", 0) * 2
    ...
    ny_rad = {
        ...
        "Total tid (sek)": total_tid_h * 3600,
        "Total tid (h)": total_tid_h,
        "Minuter per kille": tid_per_kille_min,
    }

    # Säkerställ att alla kolumner finns i ny_rad
    for col in COLUMNS:
